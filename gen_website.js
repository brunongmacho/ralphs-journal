...
const MONTH_EPIGRAPHS = {
  ...
  "2026-07": "July started with a question I couldn't answer.\nIt ended with conversations I never planned,\na stranger who felt familiar,\nand the quiet realization that\nI was no longer writing to survive.\nI was writing because I wanted to remember."
};
...
function getEntryDate(entry) {
  ...
  const m = entry.file.match(/\d{4}-\d{2}-\d{2}/);
  return m ? new Date(m[0] + "T12:00:00") : null;
}
...
function computeWeeks(entries) {
  ...
  let journalBuffer = [];
  let specialBuffer = [];
  let journalWeekKey = null;
  let weekNum = 0;

  function flushJournal() {
    if (journalBuffer.length === 0) return;
    weekNum++;
    const label = `Week ${["One", "Two", "Three", "Four", "Five"][weekNum - 1] || `Week ${weekNum}`}`;
    groups.push({ label, entries: journalBuffer, isSpecial: false });
    journalBuffer = [];
  }

  function flushSpecial() {
    if (specialBuffer.length === 0) return;
    groups.push({ label: "Special Chapters", entries: specialBuffer, isSpecial: true });
    specialBuffer = [];
  }

  for (const entry of entries) {
    if (entry.file.startsWith("SpecialChapter") || entry.file === "Introduction") {
      specialBuffer.push(entry);
      continue;
    }
    ...
    // Week boundary — flush completed week, then any buffered SCs between weeks
    if (journalWeekKey !== null && key !== journalWeekKey) {
      flushJournal();
      flushSpecial();
    }
...
const JOURNALS = [
  ...
  { file: "Journal_Day9_2026-07-02", title: "The Person I Almost Forgot" },
  { file: "Journal_Day10_2026-07-03", title: "The Day Restless Became Right" },
  { file: "SpecialChapter8_Nanay", title: "The Letters I Still Send to Heaven", date: "2026-07-03" },
  { file: "SpecialChapter9_Friends", title: "The People Who Never Let Me Disappear", date: "2026-07-04" },
  { file: "SpecialChapter10_World", title: "The World Never Stopped Loving Me", date: "2026-07-04" },
  { file: "SpecialChapter11_Courting", title: "Why I Never Stopped Courting", date: "2026-07-04" },
  { file: "SpecialChapter12_Mantra", title: "I Am Here.", date: "2026-07-04" },
  { file: "SpecialChapter13_ForgiveYourself", title: "The Things You Forgive Yourself For", date: "2026-07-04" },
  { file: "SpecialChapter14_Pets", title: "The Ones Who Loved Me Without Asking Why", date: "2026-07-04" },
  { file: "SpecialChapter15_WhatLoveBecame", title: "What Love Became", date: "2026-07-04" },
  { file: "Journal_Day11_2026-07-04", title: "The Day Love Changed Its Name" },
  { file: "Journal_Day12_2026-07-05", title: "The Day the World Felt Bigger Again" },
  { file: "Journal_Day13_2026-07-06", title: "The Day I Became Someone's Quiet Place" },
  { file: "Journal_Day14_2026-07-07", title: "The Day Life Started Throwing Curveballs" },
  { file: "Journal_Day15_2026-07-08", title: "The Day I Found Myself on the Other Side" },
  { file: "Journal_Day16_2026-07-09", title: "The Day I Made Room" },
  { file: "Journal_Day17_2026-07-10", title: "The Day Life Started Giving Back" },
  { file: "Journal_Day18_2026-07-11", title: "The Day You Let Me Into Your World" },
  { file: "Journal_Day19_2026-07-12", title: "The Day Conversations Found Me" },
  { file: "Journal_Day20_2026-07-13", title: "The Day I'm Choosing Permanent" },
  { file: "Journal_Day21_2026-07-14", title: "The Day Ordinary Was Enough" },
  { file: "SpecialChapter16_Hana", title: "The Stranger Who Felt Familiar", date: "2026-07-15" },
];
...
function generateIndex(entries) {
  ...
  for (const year of years) {
    ...
    for (const key of yearMonths) {
      ...
      for (const week of computeWeeks(byMonth[key])) {
        if (week.entries.length === 0) continue;

      body += `  <div class="week-section${week.isSpecial ? " special" : ""}">\n`;
